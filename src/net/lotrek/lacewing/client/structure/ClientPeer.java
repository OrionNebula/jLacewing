...
import net.lotrek.lacewing.client.packet.ClientWritePacket3BinaryPeerMessage;
...
public class ClientPeer
{
	...
	{
		return "ClientPeer { name: \"" + this.getName() + "\" id: " + peerID + " }";
	}
	 ...
	 */
	public void sendBinaryMessage(LacewingClient lc, int subchannel, ClientChannel channel, byte[] data) throws LacewingException
	{
		...
		try {
			ClientWritePacket.writePacketToStream(lc.getOutputStream(), new ClientWritePacket3BinaryPeerMessage(subchannel, channel.getChannelID(), getPeerID(), data));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	 ...
	 */
	public static ClientPeer getPeer(LacewingClient lc, int id)
	{
		if(!lc.knownPeers.containsKey(id))
			lc.knownPeers.put(id, new ClientPeer(id));
	 ...
	 */
	public static ClientPeer readPeerFromJoinChannel(LacewingClient lc, InputStream is) throws IOException
	{
		...
		ClientPeer toReturn = getPeer(lc, id);
